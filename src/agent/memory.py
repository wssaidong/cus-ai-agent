...
此模块提供：
1. InMemorySaver - 内存中的状态保存器 (LangGraph 1.0+)
2. MemoryManager - 记忆管理器，用于查询、清空、导出记忆
3. 会话级别的记忆隔离

注意：LangGraph 1.0 中 MemorySaver 已重命名为 InMemorySaver
"""
...
class MemoryManager:
    ...
    使用 LangGraph 1.0+ 的 InMemorySaver
    """
    ...
    def __init__(self):
        ...
        app_logger.info("记忆管理器初始化完成 (使用 InMemorySaver)")
...
def get_memory_saver() -> InMemorySaver:
    """获取 InMemorySaver 实例 (LangGraph 1.0+)"""
