...
class MCPToolsFilter(BaseModel):
    """MCP 工具过滤配置"""
    include: Optional[List[str]] = Field(default=None, description="白名单：只包含指定的工具")
    exclude: Optional[List[str]] = Field(default=None, description="黑名单：排除指定的工具")
...
class MCPServerConfig(BaseModel):
    ...
    tools: Optional[MCPToolsFilter] = Field(default=None, description="工具过滤配置")
...
class MCPConfigManager:
    ...
    def should_include_tool(self, server_name: str, tool_name: str) -> bool:
        ...
        # 如果配置了 include 白名单，只包含白名单中的工具
        if tools_filter.include:
            return tool_name in tools_filter.include

        # 如果配置了 exclude 黑名单，排除黑名单中的工具
        if tools_filter.exclude:
            return tool_name not in tools_filter.exclude
